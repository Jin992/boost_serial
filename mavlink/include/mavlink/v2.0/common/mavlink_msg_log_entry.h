...
typedef struct __mavlink_log_entry_t {
 uint32_t time_utc; /*< [s] UTC timestamp of logger since 1970, or 0 if not available*/
 uint32_t size; /*< [bytes] Size of the logger (may be approximate)*/
 uint16_t id; /*<  Log id*/
 uint16_t num_logs; /*<  Total number of logs*/
 uint16_t last_log_num; /*<  High logger number*/
}) mavlink_log_entry_t;
